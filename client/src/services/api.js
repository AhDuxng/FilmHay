import axios from 'axios';

const API_ROOT_URL = import.meta.env.VITE_VSMOV_ROOT_URL || 'https://vsmov.com/api';
const API_BASE_URL = import.meta.env.VITE_VSMOV_API_URL || API_ROOT_URL;

const api = axios.create({
  baseURL: API_BASE_URL,
  timeout: 20000,
  headers: {
    accept: 'application/json',
  },
});

const rootApi = axios.create({
  baseURL: API_ROOT_URL,
  timeout: 20000,
  headers: {
    accept: 'application/json',
  },
});

api.interceptors.response.use(
  (response) => response.data,
  (error) => {
    const status = error?.response?.status;
    const message = error?.response?.data?.message || error?.message || 'Yêu cầu thất bại';

    if (status >= 500) {
      return Promise.reject(new Error('Máy chủ đang bận. Vui lòng thử lại.'));
    }

    return Promise.reject(new Error(message));
  }
);

rootApi.interceptors.response.use(
  (response) => response.data,
  (error) => {
    const status = error?.response?.status;
    const message = error?.response?.data?.message || error?.message || 'Request failed';

    if (status >= 500) {
      return Promise.reject(new Error('Server is busy. Please retry.'));
    }

    return Promise.reject(new Error(message));
  }
);

const withPage = (page = 1) => ({
  params: {
    page,
  },
});

export const movieApi = {
  getHome: () => api.get('/danh-sach/phim-moi-cap-nhat', withPage(1)),

  getMovieList: (slug = 'phim-moi-cap-nhat', page = 1) =>
    slug === 'phim-moi-cap-nhat'
      ? api.get('/danh-sach/phim-moi-cap-nhat', withPage(page))
      : api.get(`/danh-sach/${slug}`, withPage(page)),

  getBrowseList: (page = 1, filters = {}) =>
    api.get('/danh-sach', {
      params: {
        page,
        ...filters,
      },
    }),

  searchMovies: (keyword, page = 1, filters = {}) =>
    api.get('/tim-kiem', {
      params: {
        keyword,
        page,
        ...filters,
      },
    }),

  suggestMovies: async (keyword, limit = 8) => {
    const payload = await api.get('/tim-kiem', {
      params: {
        keyword,
        page: 1,
        limit,
      },
    });

    const items = payload?.data?.items || [];

    return {
      ...payload,
      data: {
        ...payload?.data,
        items: items.slice(0, limit),
      },
    };
  },

  getGenreList: () => rootApi.get('/the-loai'),
  getMoviesByGenre: (slug, page = 1) => api.get(`/the-loai/${slug}`, withPage(page)),

  getCountryList: () => rootApi.get('/quoc-gia'),
  getMoviesByCountry: (slug, page = 1) => api.get(`/quoc-gia/${slug}`, withPage(page)),

  getYearList: () => rootApi.get('/nam-phat-hanh'),
  getMoviesByYear: (year, page = 1) => api.get(`/nam/${year}`, withPage(page)),

  getMovieDetail: (slug) => api.get(`/phim/${slug}`),
  getMovieImages: (slug) => api.get(`/phim/${slug}/images`),
  getMoviePeoples: (slug) => api.get(`/phim/${slug}/peoples`),
  getMovieKeywords: (slug) => api.get(`/phim/${slug}/keywords`),
};

export default api;
