...
import { buildMetaText, getThumbUrl, getYearFromMovie, handleImageError } from '@/shared/lib/helpers';

const HeroSection = ({ movies = [], movie: fallbackMovie, cdnBase = '' }) => {
    ...
    <section className="relative flex min-h-[560px] h-[78svh] md:h-[85vh] md:min-h-[640px] w-full items-center overflow-hidden">
      <div className="absolute inset-0 z-0">
        {slides.length ? (
          ...
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,#8766a4_0,#252526_45%,#1e1e1e_100%)]" />
        )}
        <div className="absolute inset-0 hero-gradient"></div>
      </div>
      <div className="on-dark relative z-10 mx-auto w-full max-w-container-max px-margin-mobile pt-24 md:px-margin-desktop md:pt-20">
        <div className="max-w-2xl">
          ...
          <div className="flex flex-wrap items-center gap-3 sm:gap-4">
            <Link to={movie?.slug ? `/movie/${movie.slug}` : '/search'} className="flex min-h-12 items-center gap-2 fill-ramp rounded-lg px-5 py-3 font-headline-md text-base font-bold sm:px-7">
      ...
      <div className="on-dark absolute bottom-32 right-0 bg-surface-container-low/80 backdrop-blur-md border-l-4 border-secondary px-4 py-2 font-body-md text-body-md font-bold text-on-surface hidden md:block text-white">
        {movie?.quality || 'HD'}
      </div>
    </section>
  );
