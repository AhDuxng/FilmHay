...
export const MovieInfo = ({ movie, cdnBase = '', peoples = [], keywords = [] }) => {
  ...
  return (
    <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop py-16 grid grid-cols-1 lg:grid-cols-3 gap-16">
      <div className="lg:col-span-2 space-y-16">
        <section>
          <h2 className="font-headline-lg text-headline-lg text-white mb-6">Nội dung phim</h2>
          <div
        ...
        <section>
          <h2 className="font-headline-lg text-headline-lg text-white mb-6">Diễn viên</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
            {cast.length ? cast.map((person) => (
              <div key={`${person.tmdb_people_id || person.name}-${person.character || ''}`} className="flex flex-col gap-3 group">
                <div className="w-full aspect-square rounded-full overflow-hidden border border-white/10 group-hover:border-primary-container/50 transition-colors">
                ...
                <div className="text-center">
                  <p className="font-body-lg text-body-md text-white font-semibold line-clamp-1">{person.name}</p>
              ...
              <div key={actor} className="rounded-lg border border-white/10 bg-white/5 p-4 text-center text-white">
                {actor}
              </div>
            ))}
          </div>
      ...
      <div className="space-y-12">
        <div className="bg-[#1A1A1A] border border-white/10 rounded-xl p-6 glass-panel">
          <h3 className="font-headline-md text-body-lg text-white mb-4">Thông tin</h3>
          <ul className="space-y-4 font-body-md text-body-md">
            {directors.length ? (
              <li className="flex flex-col">
                ...
                <span className="text-white">{directors.join(', ')}</span>
              </li>
            ) : null}
            {(movie?.country || []).length ? (
              <li className="flex flex-col">
                ...
                <span className="text-white">{movie.country.map((country) => country.name).join(', ')}</span>
              </li>
            ) : null}
              ...
              <li className="flex flex-col">
                ...
                <div className="flex flex-wrap gap-2">
                  {movie.category.map((category) => (
                    <Link key={category.slug} to={`/the-loai/${category.slug}`} className="rounded bg-white/10 px-2 py-1 text-sm text-white hover:bg-white/20">
                      {category.name}
                    </Link>
                  ))}
                </div>
              ...
              <li className="flex flex-col">
                ...
                <div className="flex flex-wrap gap-2">
                  {keywords.slice(0, 12).map((keyword, index) => (
                    <span key={`${keyword.id || keyword.name || keyword.keyword}-${index}`} className="rounded bg-white/10 px-2 py-1 text-sm text-white">
