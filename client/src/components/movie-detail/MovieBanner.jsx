...
import { buildMetaText, getPosterUrl, getThumbUrl, getYearFromMovie, handleImageError } from '../../utils/helpers';
...
export const MovieBanner = ({ movie, cdnBase = '', images = [] }) => {
  ...
  return (
    <section className="relative w-full h-[70vh] min-h-[600px] flex items-end pb-12 pt-32">
      <div className="absolute inset-0 z-0">
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-[#0F0F0F] via-[#0F0F0F]/50 to-transparent w-2/3"></div>
      </div>
      <div className="relative z-10 max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop w-full flex flex-col md:flex-row gap-8 items-end">
        <div className="hidden md:block w-64 aspect-[2/3] rounded-xl overflow-hidden shadow-2xl border border-white/10 shrink-0 mb-4">
          <img 
            alt={movie.name} 
        ...
        <div className="flex-grow max-w-3xl pb-4">
          <h1 className="mb-4 line-clamp-3 font-display-lg text-4xl font-bold leading-tight text-white sm:text-5xl lg:text-6xl">{movie.name}</h1>
          <p className="mb-4 font-body-md text-body-md text-secondary-fixed-dim">{movie.origin_name || movie.name}</p>
          <div className="flex flex-wrap items-center gap-3 mb-8">
            {meta.map((item, index) => (
              <span key={`${item}-${index}`} className="text-secondary-fixed-dim px-2 py-1 bg-white/10 rounded font-label-sm text-label-sm backdrop-blur-sm">
            ...
            {categoryText ? (
              <span className="text-secondary-fixed-dim px-2 py-1 bg-white/10 rounded font-label-sm text-label-sm backdrop-blur-sm">{categoryText}</span>
            ) : null}
          </div>
